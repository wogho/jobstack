...
**치환 테스트 2종**: 지원동기·기업 관련 문단에 다음 두 테스트를 적용합니다. 판정 로직은 `${CLAUDE_SKILL_DIR}~/.hermes/skills/jobstack/templates/humanize-check.md` §1을 참조합니다(스킬 본문에 자체 정의하지 않음).
...
**defense-map 산출**: 미끼 문장·예상 질문·방어 판정의 매핑을 `${CLAUDE_SKILL_DIR}~/.hermes/skills/jobstack/docs/defense-map-schema.md`의 YAML 계약 형식(schema_version 1, `source_skill: review`)으로 산출해 mock-interview가 소비할 수 있게 합니다. 저장 경로는 `$_JS_STATE/defense-maps/<회사명>_<직무>_<YYYYMMDD>.yaml`이며, Phase 6과 동일한 Bash heredoc 방식으로 기록합니다(`answer_hint`는 사용자가 확인하기 전이면 `null` — 추정 작성 금지).
...
### 개인정보 점검 (제출 전 최종 필터)
review는 제출 전 마지막 관문이므로 PII 최종 필터 위치로 적합합니다. 다음 3종을 인라인 기준으로 검사합니다(날조·외부출처 PII 금지 원칙은 `${CLAUDE_SKILL_DIR}~/.hermes/skills/jobstack/templates/guardrails.md` §1 준수).
...
2. **날조 금지** — 원문에 없는 내용을 근거로 지적하지 않습니다(`${CLAUDE_SKILL_DIR}~/.hermes/skills/jobstack/templates/guardrails.md` §1 준수).
...
진단을 마친 뒤 AskUserQuestion으로 다음 행동을 확인합니다(구조는 `${CLAUDE_SKILL_DIR}~/.hermes/skills/jobstack/templates/ask-user-question.md` 준수 — 한 번에 하나의 질문):
...
2. **파일 영속화** — 이 산출물에 한해 **Bash 파일 쓰기를 허용**합니다(allowed-tools에 Write를 추가하지 않고, resume/cover-letter #118b와 동일한 **File output protocol**을 따릅니다). 질문 세트를 소실 없이 남기기 위해:
   ...
   EOF
   ```
3. **질문↔미끼↔방어 판정 매핑**은 Phase 4의 defense-map 산출물(`${CLAUDE_SKILL_DIR}~/.hermes/skills/jobstack/docs/defense-map-schema.md` YAML 계약)로 귀속해 mock-interview가 소비할 수 있게 합니다. 공고 기반 질문은 `location`에 "공고"를 표기합니다.
...
완료 시점에 텔레메트리 후속 이벤트를 남깁니다. 이벤트 스키마는 `${CLAUDE_SKILL_DIR}~/.hermes/skills/jobstack/docs/telemetry-events.md`를 따릅니다. **최초 진단**은 `diagnosed`(추가 필드 없음)로, **재리뷰(2차 점검)**는 `second_review` 이벤트로 emit합니다(2차 점검 요청률 = second_review/diagnosed):
```bash
# 최초 진단
