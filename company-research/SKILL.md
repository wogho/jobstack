...
> - **원칙**: 도구 한계를 막다른 안내로 끝내지 않는다 — 원인 1줄 + 필요 자료 요청을 항상 함께 제시한다. (자세한 전환 규칙은 `${CLAUDE_SKILL_DIR}~/.hermes/skills/jobstack/templates/guardrails.md` §2 참조)
...
6. **기업 평판**: 잡플래닛/블라인드 등에서의 직원 평판, 면접 후기
   - **PII 가드레일**: 리뷰를 인용할 때 작성자 닉네임·프로필 등 식별정보는 리포트·캐시에 기록하지 않습니다 — 집계 요약(불만 Top 3, 키워드 빈도)과 익명 인용만 사용합니다. (상세 규칙은 `${CLAUDE_SKILL_DIR}~/.hermes/skills/jobstack/templates/guardrails.md` §1 참조)
...
> **리뷰 PII 가드레일**: 아래 세 경로에서 리뷰를 인용할 때 작성자 닉네임·프로필 등 식별정보는 리포트·캐시에 기록하지 않습니다 — 집계 요약과 익명 인용만 사용합니다. (`${CLAUDE_SKILL_DIR}~/.hermes/skills/jobstack/templates/guardrails.md` §1)
